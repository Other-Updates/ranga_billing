...
class User extends MY_Controller {
	...
	public function user_list(){
        ...
        foreach ($list as $key=>$post) {
            $delete = '<a href="" data-id="'.$post->iUserId.'" class="removeAttr action-icon" ><i class="fa fa-remove fs-5"></i></a>';
            $edit = '<a href="" data-id="'.$post->iUserId.'" class="addAttr action-icon" data-bs-toggle="modal" data-bs-target="#kt_modal_edit_user"><i class="fa fa-edit fs-5"></i></a>';
            $row = array();
            $row[] = $sno++;   
            $row[] = $post->vName;   
            $row[] = $post->eUserType;
