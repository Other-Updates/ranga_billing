   ...
   public function __construct(){
    ...
    public function get_models(){
        ...
        foreach ($list as $key=>$post) {
            $delete = '<a href="" data-id="'.$post->iModelId.'" class="action-icon removeAttr " ><i class="fa fa-remove fs-5"></i></a>';
            $edit = '<a href="" data-id="'.$post->iModelId.'" class="action-icon addAttr" data-bs-toggle="modal" data-bs-target="#kt_modal_edit_user"><i class="fa fa-edit fs-5"></i></a>';
            $row = array();
            $row[] = $sno++;
            $row[] = $post->vModelName;   
            $row[] = $post->eStatus;
