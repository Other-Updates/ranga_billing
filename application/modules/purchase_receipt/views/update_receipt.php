    ...
    <div class="section-title b-0 p-0">Payment History</div>
        <table class="table custom-table basictable view-table">
            ...
            <tbody id='receipt_info'>
                ...
                if (isset($receipt_details[0]['receipt_history']) && !empty($receipt_details[0]['receipt_history'])) {
                    ...
                    foreach ($receipt_details[0]['receipt_history'] as $val) {
                        ...
                        <tr>
                            ...
                            <td class="rec_c_date<?php echo $val['id'] ?> text-center"><?php echo date('d-M-Y', strtotime($val['created_date'])) ?> <input type='hidden'  class="datepicker-here rec_d_date<?php echo $val['id'] ?>" name='due_date'  value="<?php echo date('d-M-Y', strtotime($val['due_date'])) ?>" />
                ...
                <td></td>
                <td></td>
                <td class="text_right total-bg"><?php echo number_format($paid, 2, '.', ',') ?></td>

                <?php if ($receipt_details[0]['inv_id'] != 'Wings Invoice') { ?>
                    <td class="text_right total-bg"><?php echo number_format($dis, 2, '.', ',') ?></td>
                    <td></td>
                    <td></td>
                <?php } else { ?>
...
<div class="card"> 
  <div class="card-body">
        ...
        ?>
        <form method="post">
            <input type="hidden" name="receipt_bill[receipt_id]" value="<?php echo $receipt_details[0]['iPurchaseOrderId'] ?>">
            <div class="section-title b-0 p-0">Purchase Invoice Details</div>
            <table class="table custom-table sales-recipt-update">
                <thead>
                <th width="5%" class="text-center">S&nbsp;No</th> 
                <th width="15%" class="text-center">PO&nbsp;No</th>
                <th width="60%" class="text-center">Purchase&nbsp;Date</th>
                <th width="20%" class="text-right">Amount</th>
                </thead>
                <tbody id='receipt_info'>
                    <tr>
                        <td class="text-center">1</td>
                        <td class="text-center"><?php echo $receipt_details[0]['vPurchaseOrderNo'] ?></td>
                ...
                <input type="hidden" class="net_cost" value="<?php echo $receipt_details[0]['fNetCost']; ?>">
...
<script type="text/javascript">
    ...
    $('#discount1').live('keyup', function () {
        total = 0;
        total = (Number($('#inv_amount').val()) - Number($('#return_amount').val())) - Number($(this).val()) - Number($('#paid1').val());
        total = total - Number($('#advance').val());
        var paid = $('#paid1').val();
        // $('#balance').val(total.toFixed(2));
        //var tt = ($(this).val() / $('#inv_amount').val()) * 100;
        ...
        $('#balance').val(total_dis_amt.toFixed(2));
        var tt = ($(this).val() / Number(net_cost).toFixed(2)) * 100;
...
<script type="text/javascript">
    ...
    $("#paid1").on('keyup', function ()
    {
        var paid = $(this).val();
        var bal = $('#balance').val();
        var net_cost = $('.net_cost').val();
        var discount = $('.discount1').val();
        total = paid + bal + discount;
...
<script>
    ...
    $(document).on("click",".payedit",function(e){
        ...
        $('#form_id').attr('action',"<?php echo $this->config->item('base_url') ?>purchase_receipt/update_receipt_payment/" + id);
