...
class Pharmacy_report extends MY_Controller {
   public function __construct(){
       if(empty($this->session->userdata('LoggedId'))){
           redirect(base_url());
        }
	    parent::__construct();
        $this->load->model('pharmacy_report_model'); 
    }
	...
	public function index()
	{
        ...
        $data['salesman'] = $this->pharmacy_report_model->get_salesman_list();
        $data['distributor'] = $this->pharmacy_report_model->get_distributor_list($branch_id);

        $this->template->write_view('content', 'pharmacy_bill_report', $data);
    ...
    public function get_reference(){
        ...
        $list=$this->pharmacy_report_model->reference_list($search_data);
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $val) {
            ...
            $view = '<a href="'.base_url('billing/pharmacy_billing/view_pharmacy_fee/').$val['iSalesOrderId'].'" data-id="'.$val['iSalesOrderId'].'" class="action-icon" ><i class="fa fa-eye fs-5"></i></a>';
            $no++;
            $row = array();
            // $row[] = $no;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->pharmacy_report_model->count_all($search_data),
            "recordsFiltered" => $this->pharmacy_report_model->count_filtered($search_data),
            "data" => $data,
        );
    ...
    public function export_sales_details()
    {
        ...
        $get_all_sales_details = $this->pharmacy_report_model->get_all_sales_gst_reports($input);
