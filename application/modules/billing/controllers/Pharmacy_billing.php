...
class Pharmacy_billing extends MY_Controller {
   ...
   public function __construct(){
       if(empty($this->session->userdata('LoggedId'))){
	    ...
	    parent::__construct();
        $this->load->model('pharmacy_billing_model');
	...
	public function index()
	{
        ...
        $this->template->write_view('content', 'pharmacy_bill', $data);
        $this->template->render();
	}

    public function add_pharmacy_fee(){
        ...
        $this->template->write_view('content', 'add_pharmacy_fees', $data);
        $this->template->render();
    }

    public function edit_pharmacy_fee($id){
        ...
        $this->template->write_view('content', 'edit_pharmacy_fees', $data);
        $this->template->render();
    }

    public function view_pharmacy_fee($id){
        $data['sales_order_id'] = $id;
        ...
        $data['customer'] = $this->order_model->get_customer();
        $data['unit'] = $this->order_model->get_unit();
        $this->template->write_view('content', 'view_pharmacy_fees', $data);
        $this->template->render();
    }

    public function pharmacy_fee_return($id){
        $data['sales_order_id'] = $id;
        $data['headoffice'] = $this->order_model->get_headoffice();
        $data['category'] = $this->order_model->get_category();
        $data['sales_order'] = $this->order_model->get_sales_details_by_id($id);
        $data['customer'] = $this->order_model->get_customer();
        $data['unit'] = $this->order_model->get_unit();
        $this->template->write_view('content', 'pharmacy_fees_return', $data);
        $this->template->render();
    }

    public function get_pharmacy_bills(){
        ...
        foreach ($list as $key=>$post) {
            ...
            $edit = '<a href="'.base_url('billing/pharmacy_billing/edit_pharmacy_fee/').$post->iSalesOrderId.'" data-id="'.$post->iSalesOrderId.'" class="action-icon" ><i class="fa fa-pencil td-icon tm2"></i></a>';
            $view = '<a href="'.base_url('billing/pharmacy_billing/view_pharmacy_fee/').$post->iSalesOrderId.'" data-id="'.$post->iSalesOrderId.'" class="action-icon" ><i class="fa fa-eye fs-5"></i></a>';
            $return = "";
            if($post->eDeliveryStatus == "Delivered"){
                $edit = "";
                $delete = "";
                $return = '<a href="'.base_url('billing/pharmacy_billing/pharmacy_fee_return/').$post->iSalesOrderId.'" data-id="'.$post->iSalesOrderId.'" class="action-icon" ><button style="font-size: 11px;padding: 0px 8px;" class="btn btn-return">Return</button></a></a>';
    ...
    public function edit_supplier(){
        ...
        $data = $this->pharmacy_billing_model->supplier_by_id($supplier_id);
    ...
    public function update_supplier(){
        ...
        $this->pharmacy_billing_model->update_supplier($id,$supplier);
