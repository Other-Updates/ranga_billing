...
//            $row[] = number_format($paid_amount - ($val['receipt_bill'][0]['receipt_paid'] + $val['receipt_bill'][0]['receipt_discount'] + $val['advance']), 2, '.', ',');
    ...
    function cash_hand_over_ajaxList_view() {
        ...
        $no = $_POST['start'];
        foreach ($list as $val) {
            $views_url = '<a href="#" data-id="'.$val['iSalesmanId'].'" data-toggle="tooltip" class="action-icon view_cashinfo" title="View" data-original-title="View" ><span class="fa fa-eye"></span></a>';
            $no++;
            $row = array();
            $row[] = $no;
            // $row[] = $val['vSalesOrderNo'];
