    ...
    public function get_purchase_order(){
        ...
        foreach ($list as $key=>$post) {
            ...
            if($post->eDeliveryStatus == "Delivered"){
                $edit = '';
                $delete = '';
                $return = '<a href="'.base_url('purchase_order/purchase_order_return/').$post->iPurchaseOrderId.'" data-id="'.$post->iPurchaseOrderId.'" class="action-icon" ><button style="font-size: 11px;padding: 0px 8px;" class="btn btn-return">Return</button></a>';
            }
